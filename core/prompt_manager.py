    ...
    def format_user_message(self, msg: Union[KiraIMMessage]) -> str:
        ...
        if isinstance(msg, KiraMessageEvent):
            if msg.is_group_message():
                ...
                return f"[received_time: {date_str} message_id: {str(msg.message.message_id)}] [group_name: {msg.message.group.group_name} group_id: {msg.message.group.group_id} user_nickname: {msg.message.sender.nickname}, user_id: {msg.message.sender.user_id}] | {msg.message_str}"
            else:
                # direct message format
                return f"[received_time: {date_str} message_id: {str(msg.message.message_id)}] [user_nickname: {msg.message.sender.nickname}, user_id: {msg.message.sender.user_id}] | {msg.message_str}"
        elif isinstance(msg, KiraIMMessage):
            if msg.is_group_message():
                # group message format
